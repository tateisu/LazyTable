...
| 目標            | 困難                                                            | 対策                                                                                                                                        |
|---------------|---------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
| 大量のセルの事前サイズ計測 | Composeで行うのは非常に困難。ベタに書くとメモリ消費が非現実的                            | 事前計測部分だけViewフレームワークを使う。テキスト計測結果をComposeと揃えるため、`androidx.compose.ui.text.Paragraph`を使うカスタムビューという~~けったいな~~モノを用意。`AnnotatedString`には対応できていない |
